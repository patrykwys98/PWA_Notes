...
async def share_note(shared_note: ShareNoteGetSchema, db: Session, user: UserSchema) -> dict:
    ...
    for user_id in shared_note.user_id:
        duplicate = db.query(Share).filter(
            and_(Share.note_id == shared_note.note_id, Share.user_id == user_id)
        ).first()
        if duplicate:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Already shared for user_id: " + str(user_id)
            )
        else:
...
async def unshare_note_for_user(shared_note: UnshareNoteSchema, db: Session, user: UserSchema) -> dict:
    ...
    db_share = db.query(Share).filter(
        and_(Share.note_id == shared_note.note_id, Share.user_id == shared_note.user_id)
    ).first()
    if not db_share:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Not shared for user_id: " + str(shared_note.user_id)
        )
    else:
