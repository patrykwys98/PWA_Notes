...
async def add_note_endpoint(note: NoteCreateSchema, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return await add_note(db=db, note=note, user=user)


# @router.get('/get-notes/')
# async def get_notes_endpoint(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
#     return await get_notes(db=db, notebook_id=notebook_id)


@router.get('/get-note/{note_id}')
async def get_note_endpoint(note_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return await get_note(db=db, note_id=note_id, user=user)


@router.delete('/delete-note/{note_id}')
async def delete_note_endpoint(note_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
# async def get_childrens_endpoint(note_id: int, db: Session = Depends(get_db)):
#     return await get_childrens(db=db, note_id=note_id)
